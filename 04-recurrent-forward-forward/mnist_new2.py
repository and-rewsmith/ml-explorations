    ...
    def forward_timestep(self, input_image, prev_activations, one_hot_labels):
        ...
        for i, (prev_act, layer, norm) in enumerate(zip(prev_activations, self.layers[:-1], self.layer_norms)):
            if i == 0:  # first layer gets the input image
                new_act = F.linear(input_image, layer.weight) + F.linear(prev_activations[i + 1], layer.weight.t())
            elif i == len(prev_activations) - 1:  # last hidden layer gets the previous layer's activation and the one-hot labels
                new_act = F.linear(prev_activations[i - 1], layer.weight.t()) + F.linear(one_hot_labels, output_layer_weights)
            else:  # other layers get activations from the layers above and below
                new_act = F.linear(prev_activations[i - 1], layer.weight.t()) + F.linear(prev_activations[i + 1], layer.weight.t())
...
def activations_to_goodness(activations):
    return [torch.sum(torch.square(act)) for act in activations]
...
def train(model, positive_images, positive_labels, negative_images, negative_labels, optimizer, device, num_iterations=8, threshold=0.5):
    model.train()
    running_loss = 0.0
    ...
    activations = [torch.zeros(positive_images.shape[0], layer.out_features, device=device) for layer in model.layers[:-1]]

    # Perform multiple iterations of the recurrent forward pass
    for _ in range(num_iterations):
        # Calculate positive goodness
        positive_activations = model.forward_timestep(positive_images, activations, positive_one_hot_labels)
        positive_goodness = activations_to_goodness(positive_activations)

        # Calculate negative goodness
        negative_activations = model.forward_timestep(negative_images, activations, negative_one_hot_labels)
        negative_goodness = activations_to_goodness(negative_activations)

        # Train each layer independently
        for i, (pos_good, neg_good, layer) in enumerate(zip(positive_goodness, negative_goodness, model.layers[:-1])):
            optimizer.zero_grad()
            layer_loss = 0
...
if __name__ == "__main__":
    ...
    for epoch in range(num_epochs):
        ...
        train(model, x_pos, y_pos, x_neg, y_neg, optimizer, device)

        # Here, you should implement the validation/testing loop
        # and any other performance metrics or logging you want to use.